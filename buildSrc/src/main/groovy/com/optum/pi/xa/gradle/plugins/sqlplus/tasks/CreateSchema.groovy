 ...
 * @since 1.0.0
 */
class CreateSchema extends DefaultTask {

  static final String DESCRIPTION = "Run SQL scripts to create schema(s)"

  @TaskAction
  run() {
    if ( ! SqlPlusRunner.runSqlFilesInDir( "${project.buildEnv.createSchemaSqlDir}", project.buildEnv) ) {
      throw new GradleException( "Error in running SQL script(s) from ${project.buildEnv.createSchemaSqlDir}" )
    }
